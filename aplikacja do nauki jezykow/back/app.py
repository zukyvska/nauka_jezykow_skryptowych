    ...
    try:
        ...
        for lesson in lessons:
            if lesson.get('id') == lesson_id:
                ...
                return jsonify({
                    "correct": is_correct,
                    "message": "✅ Dobrze!" if is_correct else f"❌ Źle! Poprawna: {correct}",
...
def check_quiz():
    try:
        ...
        return jsonify({
            ...
            "passed": percent >= 70,
            "results": results,
            "message": "🎉 Świetnie!" if percent >= 90 else 
                      "👍 Dobrze!" if percent >= 70 else 
                      "💪 Spróbuj jeszcze raz!"
        })
        ...
        for user in users:
            if user.get('id') == user_id:
                stats = {
                    ...
                    "javascript": {
                        "lessons_completed": len(user["progress"]["javascript"]["lessons"]),
...
def get_ranking():
    try:
        ...
        ranked_users = []
        for user in users:
            python_score = len(user["progress"]["python"]["lessons"]) * 10 + len(user["progress"]["python"]["quizzes"]) * 5
            js_score = len(user["progress"]["javascript"]["lessons"]) * 10 + len(user["progress"]["javascript"]["quizzes"]) * 5
...
if __name__ == '__main__':
    ...
    for filepath in [PYTHON_DATA, JS_DATA, QUIZ_DATA]:
        if not os.path.exists(filepath):
            print(f"⚠️  Ostrzeżenie: {filepath} nie istnieje!")
